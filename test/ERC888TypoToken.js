...
contract('ERC888TypoToken', function ([_, wallet1, wallet2, wallet3, wallet4, wallet5]) {
    ...
    describe('sent to wrong wallet address', async function () {
        beforeEach(async function () {
            await token.mint(tokenId, wallet1, 1000);
        });
        ...
        it('should restore funds after 1 error', async function () {
            ...
            await token.transfer(tokenId, wallet2wrong, 1, { from: wallet1 });
            await token.reclaimByReceiver(tokenId, wallet2wrong, "\x01\x00" + web3.toAscii("0x0" + originalChar),  { from: wallet2 });
            (await token.balanceOf.call(tokenId, wallet2wrong)).should.be.bignumber.equal(0);
            (await token.balanceOf.call(tokenId, wallet2)).should.be.bignumber.equal(1);

            originalChar = wallet2.charAt(40);
            wallet2wrong = wallet2.replaceAt(40, (originalChar == '0') ? '1' : '0');
            await token.transfer(tokenId, wallet2wrong, 1, { from: wallet1 });
            await token.reclaimByReceiver(tokenId, wallet2wrong, "\x01\x01" + web3.toAscii("0x0" + originalChar),  { from: wallet2 });
            (await token.balanceOf.call(tokenId, wallet2wrong)).should.be.bignumber.equal(0);
            (await token.balanceOf.call(tokenId, wallet2)).should.be.bignumber.equal(2);

            originalChar = wallet2.charAt(3);
            wallet2wrong = wallet2.replaceAt(3, (originalChar == '0') ? '1' : '0');
            await token.transfer(tokenId, wallet2wrong, 1, { from: wallet1 });
            await token.reclaimByReceiver(tokenId, wallet2wrong, "\x01\x26" + web3.toAscii("0x0" + originalChar),  { from: wallet2 });
            (await token.balanceOf.call(tokenId, wallet2wrong)).should.be.bignumber.equal(0);
            (await token.balanceOf.call(tokenId, wallet2)).should.be.bignumber.equal(3);

            originalChar = wallet2.charAt(2);
            wallet2wrong = wallet2.replaceAt(2, (originalChar == '0') ? '1' : '0');
            await token.transfer(tokenId, wallet2wrong, 1, { from: wallet1 });
            await token.reclaimByReceiver(tokenId, wallet2wrong, "\x01\x27" + web3.toAscii("0x0" + originalChar),  { from: wallet2 });
            (await token.balanceOf.call(tokenId, wallet2wrong)).should.be.bignumber.equal(0);
            (await token.balanceOf.call(tokenId, wallet2)).should.be.bignumber.equal(4);
        });
        ...
        it('should failure to recover funds twice after 1 error', async function () {
            ...
            await token.reclaimByReceiver(tokenId, wallet2wrong, "\x01\x00" + web3.toAscii("0x0" + originalChar),  { from: wallet2 }).should.be.rejectedWith(EVMRevert);
            (await token.balanceOf.call(tokenId, wallet2wrong)).should.be.bignumber.equal(0);
            (await token.balanceOf.call(tokenId, wallet2)).should.be.bignumber.equal(1);
        });
        ...
        it('should failure to recover funds after 4 errors', async function () {
            ...
            (await token.balanceOf.call(tokenId, wallet2wrong)).should.be.bignumber.equal(1);
            (await token.balanceOf.call(tokenId, wallet2)).should.be.bignumber.equal(0);
        });
    });
    ...
    describe('sent to wrong smart contract address', async function () {
        ...
        it('should restore funds after 1 error', async function () {
            ...
            await token.transfer(tokenId, contractWrong, 1, { from: wallet1 });
            await token.reclaimBySender(tokenId, contract.address, contractWrong, "\x01\x00" + web3.toAscii("0x0" + originalChar),  { from: wallet1 });
            (await token.balanceOf.call(tokenId, contractWrong)).should.be.bignumber.equal(0);
            (await token.balanceOf.call(tokenId, wallet1)).should.be.bignumber.equal(1000);

            originalChar = contract.address.charAt(40);
            contractWrong = contract.address.replaceAt(40, (originalChar == '0') ? '1' : '0');
            await token.transfer(tokenId, contractWrong, 1, { from: wallet1 });
            await token.reclaimBySender(tokenId, contract.address, contractWrong, "\x01\x01" + web3.toAscii("0x0" + originalChar),  { from: wallet1 });
            (await token.balanceOf.call(tokenId, contractWrong)).should.be.bignumber.equal(0);
            (await token.balanceOf.call(tokenId, wallet1)).should.be.bignumber.equal(1000);
            ...
            await token.transfer(tokenId, contractWrong, 1, { from: wallet1 });
            await token.reclaimBySender(tokenId, contract.address, contractWrong, "\x01\x26" + web3.toAscii("0x0" + originalChar),  { from: wallet1 });
            (await token.balanceOf.call(tokenId, contractWrong)).should.be.bignumber.equal(0);
            (await token.balanceOf.call(tokenId, wallet1)).should.be.bignumber.equal(1000);
            ...
            await token.transfer(tokenId, contractWrong, 1, { from: wallet1 });
            await token.reclaimBySender(tokenId, contract.address, contractWrong, "\x01\x27" + web3.toAscii("0x0" + originalChar),  { from: wallet1 });
            (await token.balanceOf.call(tokenId, contractWrong)).should.be.bignumber.equal(0);
            (await token.balanceOf.call(tokenId, wallet1)).should.be.bignumber.equal(1000);
        });
        ...
        it('should failure to restore funds twice after 1 error', async function () {
            ...
            await token.reclaimBySender(tokenId, contract.address, contractWrong, "\x01\x00" + web3.toAscii("0x0" + originalChar),  { from: wallet1 }).should.be.rejectedWith(EVMRevert);
            (await token.balanceOf.call(tokenId, contractWrong)).should.be.bignumber.equal(0);
            (await token.balanceOf.call(tokenId, wallet1)).should.be.bignumber.equal(1000);
        });
        ...
        it('should failure to recover funds after 4 errors', async function () {
            ...
            (await token.balanceOf.call(tokenId, contractWrong)).should.be.bignumber.equal(1);
            (await token.balanceOf.call(tokenId, wallet1)).should.be.bignumber.equal(999);
        });
    });
});
